...
const getSupabaseClient = () => {
  ...
  // Crear cliente con headers de autenticación
 ...
 * @param isMain - Si es la imagen principal
...
/**
 * Sube múltiples imágenes para un edificio
 * @param files - Array de archivos de imagen
 * @param buildingId - ID del edificio
 * @param mainImageIndex - Índice de la imagen principal
 ...
 */
export async function deleteBuildingImage(imageUrl: string): Promise<{ success: boolean; error?: string }> {
  try {
    // Extraer el path del archivo de la URL
    const url = new URL(imageUrl);
    const pathParts = url.pathname.split('/');
    const bucketIndex = pathParts.findIndex(part => part === 'building-images');
 ...
 */
export async function getBuildingImages(buildingId: string): Promise<string[]> {
  try {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.storage
      .from('building-images')
    ...
    console.error('Error inesperado obteniendo imágenes:', error);
    return [];
  }
}
