    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          {t('markLocationAndUpload', 'Marca la ubicación del edificio en el mapa y sube las fotos correspondientes.')}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-600 mb-4">
              {t('searchOrClickMap', 'Puedes buscar la ubicación escribiendo la dirección o haciendo clic en el mapa.')}
            </p>
          </div>
          ...
          <div className="relative mb-4">
            <div className="flex gap-2">
              <input
                ...
                placeholder={t('addressPlaceholder', 'Ej: Calle Mayor 123, Madrid')}
            ...
            {showSuggestions && (
              <ul className="absolute left-0 top-full w-full bg-white border border-blue-400 rounded-lg shadow-2xl mt-1 max-h-56 overflow-auto" style={{zIndex: 9999}}>
                {suggestions.length === 0 && address.trim() && (
                  <li className="px-3 py-2 text-gray-500 text-sm select-none">{t('noResults', 'No se encontraron resultados')}</li>
                )}
                {suggestions.map((s) => (
                  <li
                    key={s.place_id}
          ...
          {location && (
            <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-green-800">
                <strong>{t('selectedLocation', 'Ubicación seleccionada')}:</strong><br />
                {address && (
                  <span className="block font-semibold text-green-900 mb-1">{address}</span>
                )}
                {t('latitude', 'Latitud')}: {location.lat.toFixed(6)}<br />
                {t('longitude', 'Longitud')}: {location.lng.toFixed(6)}
              </p>
            </div>
          )}
          ...
          {/* Error de ubicación */}
          {errors.location && (
            <p className="mt-2 text-sm text-red-600">{t('locationRequired')}</p>
          )}
        </div>
        ...
        <div>
          <div className="mb-4">
            <h2 className="text-lg font-semibold text-gray-900 mb-2">
              {t('buildingPhotos', 'Fotos del Edificio')} *
            </h2>
            <p className="text-sm text-gray-600 mb-4">
              {t('uploadPhotosDesc', 'Sube fotos del edificio. La primera será la foto principal.')}
            </p>
          </div>
          ...
          {photos.length < 5 && (
            <FileUpload
              ...
              maxSizeInMB={5}
              label={t('uploadPhotos', 'Subir fotos')}
              description={t('dragPhotosOrClick', 'Arrastra fotos aquí o haz clic para seleccionar ({{remaining}} restantes)', { remaining: 5 - photos.length })}
              className="mb-6"
            />
          )}
          ...
          {photos.length > 0 && (
            <div className="space-y-4">
              <h3 className="font-medium text-gray-900">
                {t('uploadedPhotos', 'Fotos subidas')} ({photos.length}/5)
              </h3>
      ...
      <div className="flex flex-col sm:flex-row gap-3 pt-8 mt-8 border-t border-gray-200">
        ...
        >
          {t('saveAndContinue', 'Guardar y continuar')}
        </button>
      </div>
    </div>
  );
