...
// Las 8 secciones del libro digital
const BOOK_SECTIONS = [
  {
    id: 'general_data',
...
const ManualSectionsWizard: React.FC<ManualSectionsWizardProps> = ({
  ...
  // Determinar paso inicial basado en startSection
  const initialStep = startSection 
    ? BOOK_SECTIONS.findIndex(section => section.id === startSection)
    : 0;
  
  const [currentStep, setCurrentStep] = useState(initialStep >= 0 ? initialStep : 0);
  const [sectionData, setSectionData] = useState<Record<string, any>>({});
  const [sectionDocuments, setSectionDocuments] = useState<Record<string, File[]>>({});
  const [completedSections, setCompletedSections] = useState<Set<string>>(new Set());
  
  const currentSection = BOOK_SECTIONS[currentStep];
  
  // Obtener datos del estado de navegación
  // const sections = location.state?.sections || BOOK_SECTIONS;
  ...
  const validateCurrentSection = (): boolean => {
    const currentData = sectionData[currentSection.id] || {};
  ...
  const handleFinish = () => {
    ...
    console.log('Libro Digital completado:', finalData);
    
    // Navegar de vuelta al hub con mensaje de éxito
    navigate('/libro-digital/hub', {
      state: {
        buildingName,
        buildingId,
        message: '¡Libro Digital completado exitosamente!'
      }
    });
  };
        ...
        >
          ...
          <div className="space-y-6">
            ...
            <div className="border-t border-gray-200 pt-6">
              ...
              {sectionDocuments[currentSection.id] && sectionDocuments[currentSection.id].length > 0 && (
                <div className="mt-4">
                  <p className="text-sm font-medium text-gray-700 mb-2">
                    Documentos subidos ({sectionDocuments[currentSection.id].length}):
                  </p>
                  <ul className="text-sm text-gray-600 space-y-1">
                    {sectionDocuments[currentSection.id].map((file, index) => (
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-gray-200">
              ...
              <button
                type="button"
                onClick={handleNext}
                className="px-6 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 sm:ml-auto"
              >
